...
module scale::position_tests {
    ...
    fun test_close_position(){
        ...
        let _position_id_2: ID;
        let _position_id_3: ID;
        ...
        test_scenario::next_tx(tx,owner);
        {
            ...
            _position_id_2 = position::open_position<Tag,SCALE>(&mut list, market_id, &mut account, &root,100000,5,2,1,test_scenario::ctx(tx));
            _position_id_3 = position::open_position<Tag,SCALE>(&mut list, market_id, &mut account, &root,1000,2,1,2,test_scenario::ctx(tx));
        ...
        test_scenario::next_tx(tx,owner);
        {
            ...
            let p = account::get_profit(&account);
            assert!(i64::get_value(p) == 982,529);
